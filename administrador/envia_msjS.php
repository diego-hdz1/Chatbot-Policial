...
if (isset($_POST['referencia']) && isset($_POST['id_oficial']) && isset($_POST['respuesta']) && isset($_POST['seguimiento'])) {
    ...
    $query = "SELECT referenciaS FROM consultapersonas WHERE id_consulta = '".$referencia."'";
    $result = mysqli_query($conexion, $query) or die("Error en la consulta: " . mysqli_error($conexion));
    $data = mysqli_fetch_array($result);
    ...
    mysqli_query($conexion, $query);
    $query = 'UPDATE consultapersonas SET estado = 3 WHERE id_consulta = '.$referencia.'';
    mysqli_query($conexion, $query);
    ...
    if ($telefono_oficial) {
        ...
        try {
            ...
            if($seguimiento === 'si'){
                ...
                $Message = "Seguimiento Personas\nUbicación actual:\nColonia actual: \nSector: \nEdad: \nNacionalidad: \nDomicilio detenido:";                $query = 'UPDATE consultavehiculos SET estado = 5 WHERE referenciaV = '.$referencia.'';
                mysqli_query($conexion, $query);
                $query = 'UPDATE solicitudes SET estado = 5 WHERE referenciaS = '.$referenciaS.'';
                mysqli_query($conexion, $query);
                $query = 'UPDATE consultapersonas SET estado = 5 WHERE id_consulta = '.$referencia.'';
                mysqli_query($conexion, $query);
